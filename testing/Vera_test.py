...
def  test_one_map(flag):
    ...
    print("time:", ("\033[92m%s\033[0m" if timer < timer2 else "\033[91m%s\033[0m") %
          timer)
    print("time gleb:", ("\033[92m%s\033[0m" if timer2 < timer else "\033[91m%s\033[0m") %
          timer2)
    time_a.append(timer)
    time_b.append(timer2)
    if timer > 3:
        error_time += 1
...
if len(sys.argv) >= 3:
    ...
    import statistics
    print("Mean 1 = {}, mean 2 = {} 1 faster than 2  \033[91mx{:.4f}\033[0m times".format(
        statistics.mean(time_a), statistics.mean(time_b), statistics.mean(time_b) / statistics.mean(time_a)))
    print(statistics.stdev(time_a), statistics.stdev(time_b))
else:
    test_one_map(flag)